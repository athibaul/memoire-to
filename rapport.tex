...
\end{equation}
Ceci définit bien une unique valeur car $KL$ est une fonctionnelle strictement convexe.
\end{definition}

On considère $\Ccal_1, \ldots, \Ccal_l$ une famille de convexes. On prend $\xi_0$ quelconque, et on construit la suite $(\xi_i)$ par récurrence de la façon suivante~:
\[
\xi_{i+1} = P^{KL}_{\Ccal_{i+1}}(\xi_i)
\]
où les indices sont à comprendre modulo $l$. On note par la suite $P_i \xi = P^{KL}_{\Ccal_i} (\xi)$. On s'intéresse à la convergence de cette suite.
\begin{lemma}
...
À l'aide de la divergence de Kullback-Leibler, le problème de minimisation (\ref{eq:keps}) se réécrit~:
\[
...
\[
\Ccal_1 = \left\{ \gamma \in \IR_{+*}^{nm} \mid \sum_j \gamma_{i,j} = \mu_i \right\}
; \quad
\Ccal_2 = \left\{ \gamma \in \IR_{+*}^{nm} \mid \sum_i \gamma_{i,j} = \nu_j \right\}
\]
On a notamment $\Pi(\mu,\nu) = \Ccal_1 \cap \Ccal_2$.
Le minimiseur $\gamma^\epsilon$ est donc le projeté de $G$ sur $\Ccal_1 \cap \Ccal_2$ selon la divergence de Kullback-Leibler.
\end{proof}
