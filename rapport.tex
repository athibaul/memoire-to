...
On se limite dorénavant aux cas discrets. Le coût de transport pour le plan $\gamma \in \Pi(\mu,\nu)$, selon la formulation de Kantorovich, était~:
...
\begin{lemma}
Le coût $K^\epsilon$ est une fonction strictement convexe, dont l'unique solution $\gamma^\epsilon$ a tous ses coefficients non nuls.
\end{lemma}
\begin{proof}
\begin{description}
...
\end{lemma}
\begin{proof}
Notons $\bar{\gamma}$ le minimiseur de l'entropie $S$ sur le convexe $\argmin_{\Pi(\mu,\nu)} K$, et le coût correspondant $K_{opt} := K(\bar{\gamma})$.

On peut pour toute suite de $\epsilon$ tendant vers 0, extraire une sous-suite telle que $\gamma^\epsilon$ admette une limite $\gamma^0$.
Montrons que la seule limite possible est $\bar{\gamma}$.
